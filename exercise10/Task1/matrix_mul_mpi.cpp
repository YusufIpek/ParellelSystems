...
#if !defined(CONTIGUOUS_WITH_MULTIPLICATION) && !defined(CONTIGUOUS_WITH_INDIRECTION)
#define CONTIGUOUS_WITH_MULTIPLICATION
#endif


#if defined(CONTIGUOUS_WITH_MULTIPLICATION)
...
Matrix mul(const Matrix& a, const Matrix& b, int height, int width, int rowBoundaryL, int rowBoundaryH, int columnBoundaryL, int columnBoundaryH, unsigned n) {
	...
	for (int i = rowBoundaryL, ix = 0; i < rowBoundaryH; ++i, ix++) {
		for (int j = columnBoundaryL, jx = 0; j < columnBoundaryH; ++j, jx++) {
